// I have make some thought mistake in this problem
// the point is to add all word in vector 'words' into
// the trie, but not the words generated by dfs!
...
private:
  ...
  bool dfs(vector<vector<char>> &board,
    ...
    for (unsigned short i = 0; i < 4 && !find; i++){
      ...
      if (nx < 0 || ny < 0 || nx >= m || ny >= m || board[x][y] != word[0] || visit[x][y]){
        continue;
      }
      find = find || dfs(board, visit, nx, ny, word, build, tree);
    }
    visit[x][y] = false;
    cout << x << " " << y << " " << find << " " << build << endl;
    return find;
  }
...
public:
  vector<string> findWords(vector<vector<char>>& board, vector<string>& words) {
    ...
    for (int index = 1; index < words.size(); index++){
      ...
      if (lookup(toSearch, words[index].susbtr(0, samePrefixLength), board, visit, tree))
